...
if (toiletUseCount('非常用トイレ 1回分 5個 セット') !== 5) {
  issues.push('product quantity parsing failed: bundled single-use toilets must be totaled');
}
if (titleShort('非常用トイレ 1回分 5個 セット') !== '5回分 非常用トイレ') {
  issues.push('product title failed: bundled single-use toilets must show total uses');
}
if (toiletUseCount('非常用トイレ 1回分 5袋セット') !== 5) {
  issues.push('product quantity parsing failed: bundled single-use toilet bags must be totaled');
}
if (matchesPageType({ titleRaw: 'ソーラーパネル 120W ポータブル電源用 太陽光パネル' }, { slug: 'portable-power-kaigo' })) {
  issues.push('product page matching failed: standalone solar panels must be excluded from portable power comparisons');
}
if (matchesPageType({ titleRaw: 'ソーラーパネル 200W 1000Wh対応 ポータブル電源用' }, { slug: 'portable-power-kaigo' })) {
  issues.push('product page matching failed: compatible-capacity text must not make a standalone panel eligible');
}
if (!matchesPageType({ titleRaw: 'ポータブル電源 1070Wh 定格1500W ソーラーパネルセット' }, { slug: 'portable-power-kaigo' })) {
  issues.push('product page matching failed: portable power and solar panel bundles must remain eligible');
}
if (titleShort('Jackery Solar Generator 1000 New 1070Wh 100W ポータブル電源 ソーラーパネル セット 定格1500W') !== 'Jackery 1070Wh 1500W ソーラーパネルセット ポータブル電源') {
  issues.push('product title failed: rated output must take priority over solar panel wattage');
}
if (titleShort('Jackery Solar Generator 1000 New 1070Wh 100W ポータブル電源 ソーラーパネル セット') !== 'Jackery 1070Wh ソーラーパネルセット ポータブル電源') {
  issues.push('product title failed: panel wattage must be omitted when station output is unknown');
}
if (titleShort('Jackery 1070Wh ポータブル電源 ソーラーパネルセット パネル定格出力100W') !== 'Jackery 1070Wh ソーラーパネルセット ポータブル電源') {
  issues.push('product title failed: panel rated output must not be treated as station output');
}
if (titleShort('Jackery 1070Wh ポータブル電源 ソーラーパネルセット パネル：定格出力100W') !== 'Jackery 1070Wh ソーラーパネルセット ポータブル電源') {
  issues.push('product title failed: punctuated panel output must not be treated as station output');
}
...
if (paidProductEnabled) {
  if (!fs.existsSync(paidProductPath)) {
    ...
    const requiredMarkers = [
      ...
      'assets/paid-kit/inventory-ledger.png',
      'assets/paid-kit/order-candidates.png',
      'assets/paid-kit/approval-summary.png',
      'assets/paid-kit/expiry-management.png',
      '08 出典と確認',
      'id="planSummary"',
...
for (const name of ['basic-input.png', 'inventory-gap.png', 'inventory-ledger.png', 'order-candidates.png', 'approval-summary.png', 'expiry-management.png']) {
